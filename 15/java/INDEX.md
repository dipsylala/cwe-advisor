...
- Treat a request-reachable `System.setProperty()` as high severity regardless of the value written: it can disable the deserialization filter (`jdk.serialFilter`), repoint TLS trust (`javax.net.ssl.trustStore`), redirect outbound traffic (`http.proxyHost`, `https.proxyHost`, `http.nonProxyHosts`), or re-enable JNDI remote codebase loading (`com.sun.jndi.ldap.object.trustURLCodebase`)

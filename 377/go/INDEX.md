...
```go
// SAFE: os.CreateTemp generates a random name with 0600 permissions atomically
package main
...
func processSensitiveData(data []byte) error {
    f, err := os.CreateTemp("", "secure-*.tmp") // TMPDIR/TEMP respected; mode 0600
    if err != nil {
        return fmt.Errorf("create temp file: %w", err)
    }
    defer os.Remove(f.Name()) // SAFE: guaranteed cleanup on all paths
    defer f.Close()

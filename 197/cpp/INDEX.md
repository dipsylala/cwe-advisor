...
C++ inherits C's implicit narrowing in most contexts - `static_cast`, function-style casts, ordinary assignment - so the same silent bit-loss applies to `int64_t` to `int32_t` and `double` to `int`. Brace initialization (`int x{value};`) is the one real improvement: it is a compile error when the initializer would narrow, which makes a subset of these impossible to write by accident, though it reports where the conversion is rather than whether the value fits. Everywhere else a validated conversion is still needed, and `static_cast` documents intent without performing any check.
...
- Use brace initialization for new code so accidental narrowing fails to compile
...
- Build with `-Wconversion -Wfloat-conversion -Wsign-compare` (and `-Werror` in new code), and run tests under `-fsanitize=undefined`
...
- Harden configuration - enable the conversion warnings, prefer brace initialization, and build tests with `-fsanitize=undefined`

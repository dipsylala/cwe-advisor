...
- `PKIXRevocationChecker` (Java 8+) is the explicit form: it lets you choose `Option.PREFER_CRLS`, `NO_FALLBACK`, and - deliberately, never by default - `SOFT_FAIL`. It also has `ONLY_END_ENTITY`, which checks the leaf certificate but not the intermediates - a revoked intermediate (a CA that lost control of a signing key) is exactly the case revocation checking exists to catch, so leave the full chain checked unless there's a specific reason not to
- Wiring a `PKIXRevocationChecker` onto a standalone `PKIXBuilderParameters`/`PKIXParameters` object that is never attached to the connection is a no-op fix: it has to reach `TrustManagerFactory.getInstance("PKIX")` via `init(new CertPathTrustManagerParameters(pkixParams))`, and that factory's trust managers have to be the ones passed to `SSLContext.init()` - build the checker, attach it, and confirm the resulting `SSLContext` is the one actually used to make connections
...
- OCSP stapling (`jdk.tls.client.enableStatusRequestExtension`) avoids the responder round trip and the availability problem it introduces - it already defaults to `true` on JDK 9+, so setting it is a no-op there; only pre-9 JDKs need it set explicitly. The server-side counterpart, `jdk.tls.server.enableStatusRequestExtension`, defaults to `false` - relevant when the code under review is itself a TLS server or terminator, not just a client
...
- Test - connect to a deliberately revoked test endpoint and assert the handshake fails with `CertPathValidatorException.getReason() == BasicReason.REVOKED` specifically, not just any failure - a generic handshake failure equally means the test CA itself isn't trusted, which would pass this test for the wrong reason; then connect to a valid endpoint and assert it succeeds

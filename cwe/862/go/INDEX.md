...
- gRPC has no HTTP status to return: every RPC answers `200` with the outcome in the `grpc-status` trailer. Enforce there with a `grpc.UnaryServerInterceptor`/`StreamServerInterceptor` installed via `grpc.ChainUnaryInterceptor` and `grpc.ChainStreamInterceptor` respectively, denying with `status.Errorf(codes.PermissionDenied, ...)` - or `codes.Unauthenticated` where the caller could not be identified

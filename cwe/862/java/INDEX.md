...
- Add resource-based authorization - For entity-specific actions, reference a security bean from the SpEL expression that verifies the authenticated user owns or has a granted relationship to the specific record; the `@beanName.method(...)` reference resolves against the bean's name, which defaults to the uncapitalized class name (`OrderSecurity` becomes `orderSecurity`), so name it explicitly only when the class name does not match the expression. That bean answers 403, which is right for a role or relationship decision; for ownership of a guessable identifier prefer the owner-scoped repository query (`findByIdAndOwnerId`) whose empty result is the 404 the test below asserts

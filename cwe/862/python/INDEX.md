...
- Add object-level authorization - Scope `get_queryset()` to the requesting user first: `get_object()` filters the queryset before any object permission runs, so this is what makes a foreign id answer with the same 404 as a missing one, and it is the only step that produces the status the test below asserts. Add a custom `BasePermission` with `has_object_permission(self, request, view, obj)` comparing `obj.owner_id` to `request.user.id` as the second line for detail actions - DRF's generic views call it automatically via `get_object()`, but over an unscoped queryset it answers 403 and rebuilds the existence oracle

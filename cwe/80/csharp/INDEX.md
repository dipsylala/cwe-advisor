...
- Sanitize rich content with the `HtmlSanitizer` package (namespace `Ganss.Xss` from 8.0.601; the package id is `HtmlSanitizer`, not the namespace). **Operative floor 9.2.1039.** The advisory-derived floor is only 9.0.967, but this package's advisory record undercounts it twice over: the highest-severity issue, a CSS-removal denial of service fixed in 9.0.967, carries no CVE at all, and the `SanitizeDom` wrapper-attribute bypass fixed in 9.2.1039 was never filed as an advisory. Note the shape of the rest of the history too - the style-tag, foreign-content and template-tag bypasses were all unlocked by *permissive configuration*, which is exactly what a rich-text editor integration does
...
- For rich text, sanitize with `HtmlSanitizer` at 9.2.1039 or later, and review any element or attribute added to its allowlist against the bypass history above

...
LDAP Injection occurs when untrusted user input is concatenated into LDAP queries without proper sanitization, allowing attackers to modify query logic. This can lead to authentication bypass, unauthorized data access, or privilege escalation in applications using LDAP for directory services. The core fix is to use LDAP filter builders or RFC 4515 escaping; input validation is a separate decision, for values whose format the application defines.
...
- Filter objects or escaping close the injection; add an allowlist only where the application defines the value's format (a username policy) and say what it rejects
...
- Where a username or search-term policy exists, enforce it before the query and state it in the write-up; do not add a pattern for the fix alone

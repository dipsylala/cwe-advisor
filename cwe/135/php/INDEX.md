...
- Use `mb_strlen($s, 'UTF-8')` for a user-facing length limit and `strlen($s)` for a storage-size limit - keep both, and do not let one stand in for the other
...
- Database column limits are usually bytes (`VARCHAR(255)` in `utf8mb4` is 255 characters but the row limit is bytes) - check both before writing

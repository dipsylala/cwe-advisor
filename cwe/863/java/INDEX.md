...
- Use `@PreAuthorize` with a SpEL expression that combines role and ownership, e.g. `hasRole('ADMIN') or (hasRole('USER') and @orderSecurity.isOwner(#id, authentication.name))`, rather than a role check alone. Keep a role on the ownership branch: a bare `hasRole('ADMIN') or isOwner(...)` grants every authenticated owner and so widens an admin-only endpoint, and whether owners may act at all is a product decision the fix must not make. `@PostAuthorize` is not a substitute on a method that writes: it runs after the body, so the change is already made when the check fails, and Spring warns against pairing it with `@Transactional`
...
- Where the expression is a role-or-permission disjunction, granting the permission to the role through a `RoleHierarchy` bean (`RoleHierarchyImpl.fromHierarchy("ROLE_ADMIN > permission:read")`, Spring Security 6.3+ - `new RoleHierarchyImpl()` plus `setHierarchy(String)` before that) keeps the rule in one place instead of in every annotation
...
- Implement ownership checks against the repository rather than a cached or client-asserted value. For a guessable identifier prefer the owner-scoped query (`findByIdAndOwnerId`) whose empty result becomes a 404, matching CWE-862 and the siblings here; a SpEL bean that loads by id then compares answers 403, telling the caller the record exists
...
- Test - Add `@WithMockUser` MockMvc tests for a non-owner with a valid role, and for a role not in the allowlist, confirming both are denied. Note what the test type returns: through MockMvc a `@PreAuthorize` denial is a 403, while invoking the annotated bean directly raises `AccessDeniedException`. Where the ownership check is the owner-scoped repository query above, the non-owner gets the same 404 a missing id does - assert that, and assert the bodies match. `@WithMockUser(roles = "X")` grants `ROLE_X`; `authorities = "X"` grants `X` verbatim

...
- Bind, encode, validate, or authorize - Scope `get_queryset()` to the requesting user, which is what makes the non-owner's request answer with the same 404 a missing id does, since `get_object()` filters the queryset before any object permission runs; add `has_object_permission` comparing `obj.owner_id == request.user.id` alongside the role check as the second line, remembering that over an unscoped queryset it answers 403 and rebuilds the existence oracle

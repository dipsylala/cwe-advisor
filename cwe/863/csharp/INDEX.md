...
- Test - Add tests for an authenticated non-owner requesting another user's resource, and for a role value the allowlist does not recognize. Expect 403 for the unrecognised role; where the caller named a guessable identifier, expect the same 404 an unknown identifier returns, with a matching body, since a 403 there confirms the record exists

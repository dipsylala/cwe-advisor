...
- Always pass the flags explicitly rather than relying on the default, and note which way the version cuts: before PHP 8.1 the default was `ENT_COMPAT`, which leaves the single quote unescaped, so a value interpolated into a single-quoted attribute can break out of it. PHP 8.1 changed the default to `ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401`, but code that runs on both needs the flags written out

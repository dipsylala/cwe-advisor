...
- Encoding and sanitizing answer different questions. Encode when the value is text. When the value is meant to carry markup - a field named `bodyHtml`, a rich-text editor's output, a CMS body - swapping `th:utext` for `th:text` closes the XSS by printing the tags as literal text, a functional regression that is silent unless the write-up says so. For markup, sanitize with the OWASP Java HTML Sanitizer (artifact `com.googlecode.owasp-java-html-sanitizer:owasp-java-html-sanitizer`, date-versioned - set the floor at `20260101.1`, which closed CVE-2025-66021 against `20240325.1`, and `20260313.1` is the current release; its classes live in package `org.owasp.html`, and an import built from the group id such as `com.googlecode.owasp.java_html_sanitizer.PolicyFactory` does not compile) using an explicit `PolicyFactory` such as `Sanitizers.FORMATTING.and(Sanitizers.BLOCKS)`, and keep `th:utext` for the sanitized result. The Java Encoder has no method for this - it escapes, it does not allowlist tags

...
- Use structured JSON/ECS logging, or explicitly encode the whole ASCII control range (`\x00-\x1F`, `\x7F`) plus the Unicode separators U+0085, U+2028 and U+2029, and the backslash itself - without escaping the backslash, an attacker who types the two characters `\` and `n` produces output identical to a real newline and the log stops recording which arrived

...
- Derive the stored extension from the Tika-detected type - a fixed map, or `MimeTypes.getDefaultMimeTypes().forName(type).getExtension()` (`org.apache.tika.mime`) - rather than from `getOriginalFilename()`. `forName()` throws the checked `MimeTypeException` only for a malformed media-type string such as `notatype`; a well-formed but unregistered type like `application/x-nonexistent` is minted and returned, with `getExtension()` empty. So the empty-string check is the one that catches an unknown type, and a `catch (MimeTypeException)` written to handle "unrecognized" never fires. Check both before appending to a filename - an unguarded call can produce a name ending in a bare dot. A random name with a client-chosen suffix still leaves the client deciding how the stored file will be served
- For image uploads, re-encode with `javax.imageio.ImageIO` (read then write) before persisting, which strips embedded scripts or malformed metadata that raw bytes may carry; `ImageIO.read()` returns `java.awt.image.BufferedImage`, which is not in `javax.imageio`
- When serving a stored file back, set `Content-Type` from the detected type and `Content-Disposition: attachment` for anything a browser would render inline (SVG, HTML). `ResponseEntity.ok()` returns `ResponseEntity.BodyBuilder`, and `HttpHeaders` has no constant for `X-Content-Type-Options` - pass that header name as a string with the value `nosniff`

...
- Serve stored files back through a handler that sets `Content-Type` from the detected type and `X-Content-Type-Options: nosniff`, adding `Content-Disposition: attachment` for the types the root entry lists as browser-rendered documents

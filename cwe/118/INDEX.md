...
MITRE's broad umbrella for out-of-range access to an indexable resource, and discouraged for direct mapping: prefer the descendant - an out-of-bounds read is CWE-125, a write CWE-787, a stack-based overflow CWE-121, and uninitialized pointer access CWE-824. Where the untrusted value is the index itself rather than a missing check on the access, CWE-129 is the closer fit, which MITRE files outside this hierarchy because the root cause there is missing input validation.

...
- Restricting `eval`/`exec`'s `globals`/`locals` (e.g. `{"__builtins__": {}}`) is not a control against untrusted input - it is bypassable via `().__class__.__base__.__subclasses__()` regardless of what's in scope, since that chain reaches arbitrary classes without needing a name from the restricted namespace at all. Treat any `eval`/`exec` on untrusted input as unfixable in place, not as hardenable
- An argument-list `subprocess.run([...])` call closes shell metacharacter injection but not option injection: a value starting with `-` can still be read as a flag by the target program. `bandit` stays silent on this - a clean run is not evidence the argument itself is safe
...
- Test - Run payloads such as `__import__('os').system('id')` and `().__class__.__base__.__subclasses__()` against replaced `eval`/`exec` calls, and shell metacharacters (`; | & $() \``) plus a leading-`-` argument against replaced subprocess calls, confirming all are rejected or treated as literal data

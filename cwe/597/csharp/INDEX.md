...
- For a secret - a token, an API key, an HMAC - correcting the comparison mode is only half the fix: `CryptographicOperations.FixedTimeEquals()` takes two `ReadOnlySpan<byte>` arguments, not strings, and Microsoft documents that it short-circuits (leaking the length difference) when the two spans are not the same length - hash both values to a fixed length first (e.g. `SHA256.HashData(Encoding.UTF8.GetBytes(...))`) so the comparison always walks the same number of bytes, then compare the digests with `FixedTimeEquals()`

...
- Test - confirm the probe that executed before the fix is now refused rather than merely failing differently: against the AST allowlist, `__import__('os')` is rejected at parse time as a forbidden `Call` node, and a legitimate formula such as `price * qty` still evaluates

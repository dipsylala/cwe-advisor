...
Decide which half the finding is before choosing a fix: a credential the product *sends* belongs in a managed store, while a credential the product *accepts* has no correct home, because relocating it leaves the same authenticator working on every installation. Where the finding is a default account or password shipped with the product rather than one embedded in code, that is CWE-1392/CWE-1393.

...
- Environment variables are a fallback rather than a store, but state the exposure accurately: `/proc/<pid>/environ` is governed by a `PTRACE_MODE_READ_FSCREDS` check, so it is readable by the same user or by a process holding `CAP_SYS_PTRACE`, not by any local account. The realistic paths are a compromised process running as that user, a crash report, a platform metadata endpoint, and `docker inspect`, which Docker's own reference names as where `ENV` values can be viewed - which is why Compose and Swarm document secrets as files under `/run/secrets/` instead

...
- A JWT signing key hardcoded as a literal is this same weakness with a wider blast radius: it forges every token the application will ever issue, not the one credential that leaked. Load it the same way as any other secret; see CWE-330 for how large it needs to be, since `golang-jwt` enforces no minimum length on it at all

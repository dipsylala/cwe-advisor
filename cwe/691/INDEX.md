...
MITRE marks this Pillar Discouraged, so file a descendant: a race an attacker interleaves with is CWE-362, control leaving the frame meant to contain a failure is CWE-248, workflow ordering is CWE-841, a dispatch with no handler at all is CWE-431, and a handler that runs and is the wrong one is CWE-430. A return value nobody inspects is not a descendant of this pillar at all - it is CWE-252, under CWE-703 - and that is the shape findings most often arrive here as.

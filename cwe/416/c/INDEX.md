...
- Null the pointer at the moment of release, through a release function that takes the address of the caller's variable (`void close_session(struct Session **sp)`), and open that function with `if (sp == NULL || *sp == NULL) return;` so a second call is a no-op rather than a double free
...
- Where a pointer must outlive the call that hands it out - a deferred callback, a queued job - pass a handle the receiver can revalidate rather than a raw address. A bare index is not a handle: once the slot is reused the index resolves to a different, live object and the revalidation passes. Store a generation counter in the owner's slot table, increment it both on release and on re-acquire, pass index plus generation, and accept only when both match; size the counter so it cannot wrap within the process lifetime. The table itself is then indexed by a number that came from outside, so bounds-check the index on the release path as well as the read path - `slots[index]` is pointer arithmetic like any other, and a fix for this weakness that opens CWE-823 has moved the bug rather than closed it. The counter lives beside the pointer in the table, never inside the object it validates: a check written as `ctx->conn->generation == expected` reads the freed block through the stale pointer, which is the use-after-free the check was meant to prevent, relocated rather than removed
...
- Test - exercise the early-release path and the normal path in the same run, and confirm ASan reports no use-after-free; assert that a second call to the release function is a no-op. Where the fix is a generation handle, test it against a slot that has been *reused* rather than left empty: acquire, release, acquire a second object into the same slot, then resolve the first handle and confirm it is refused. A revalidation that only checks the slot is occupied passes while the slot is empty and fails here, which is the case the counter exists for

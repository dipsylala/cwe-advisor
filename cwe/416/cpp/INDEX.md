...
- A lambda capturing by reference, or capturing a raw `this`, outlives the frame it referred to when it is stored in a callback or queue; capture a `shared_ptr` (or `weak_ptr`) instead. Inside a member function that means deriving the class from `std::enable_shared_from_this` and capturing `shared_from_this()` - a bare `this` carries no ownership, and `shared_from_this()` throws `std::bad_weak_ptr` (C++17 and later; undefined behaviour before that) if the object is not already owned by a `shared_ptr`. Where the callback need not be copyable, `std::move_only_function` (C++23) allows a `unique_ptr` to be moved into the capture
- Invalidation rules are per-container, not universal: `vector`/`deque`/`string` can invalidate every outstanding iterator and reference on an insertion that changes capacity, and always invalidate from the erased element onward; `list`, `map`/`set`, and the unordered associative containers only invalidate an iterator or reference to the element actually erased - insertion into these leaves every other iterator and reference valid. Check the specific container's own invalidation guarantees rather than assuming either behaviour applies generally
- Build tests with `-fsanitize=address` and the standard library's hardened mode; add `-Wdangling-reference` (GCC 13+, in `-Wall`) or Clang's `-Wdangling` family, which catch a returned reference or a view of a temporary at compile time - often the only place they can be caught, since a stale `string_view` frequently still reads the old bytes correctly at runtime. Valgrind's heap tracking does not extend to stack frames, so it misses exactly this class
...
- Harden configuration - build and test with `-fsanitize=address`, `-D_GLIBCXX_ASSERTIONS` (or the libc++/MSVC equivalent), which also catches invalidated-iterator use. Plain ASan red-zones heap blocks, so add `-fsanitize-address-use-after-scope` and run with `ASAN_OPTIONS=detect_stack_use_after_return=1` - a view or reference that outlived a stack frame is the shape this entry calls dominant, and it is a stack access ASan has no reason to object to otherwise
- Test - exercise the path where the owner is released while an observer is still registered, and confirm the observer's access is refused rather than performed. For an alias into a container, assert against a container that has actually reallocated - `reserve` a small capacity, take the reference, then push past it - since a `vector` that never outgrows its initial buffer keeps every reference valid and the test passes for the wrong reason

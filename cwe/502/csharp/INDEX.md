...
- Establish which target framework the code builds for before proposing the fix: `BinaryFormatter` is obsolete from .NET 5, and from .NET 9 the in-box implementation throws on use with the compatibility switches that previously re-enabled it removed. On .NET 9+ the finding is a runtime failure rather than a live vulnerability, and re-enabling is not an option to offer. The reverse holds on .NET Framework 4.8, which is still supported and where the same code is fully exploitable - so the runtime decides whether this is urgent or already dead

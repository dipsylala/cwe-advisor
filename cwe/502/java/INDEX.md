...
**Primary Defence:** Use JSON (Jackson, Gson) instead of Java serialization, or if Java serialization is required, implement `ObjectInputFilter` to allowlist permitted classes. Its floor is not JDK 9: JEP 290 was backported to 8u121, so `ValidatingObjectInputStream` (Apache Commons IO) is needed only below that.
...
- If Java serialization is unavoidable, implement `ObjectInputFilter` with an explicit per-class allowlist; `ValidatingObjectInputStream` (Apache Commons IO) is the fallback only below 8u121

...
- Encode the byte array as a token using `Convert.ToBase64String()` (strip `=` padding and map `+`/`/` to `-`/`_` for base64url) or `Convert.ToHexString()`
- For integer ranges (OTP, PIN), use `RandomNumberGenerator.GetInt32(int toExclusive)` or the `GetInt32(fromInclusive, toExclusive)` overload (.NET 6+)

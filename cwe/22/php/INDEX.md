...
- Break taint after the containment check - pass the map's value (or the `realpath()` result) to the sink, never the original request value
- Harden configuration - add `open_basedir` for the application's directories and apply the containment check to every entry in a hand-rolled archive extraction loop, `ZipArchive::extractTo()` itself strips a leading `/` and any `..` element from member names, so an entry named `../../evil.txt` lands inside the destination (verified on PHP 8.5; the manual documents none of this, so do not infer it from the docs) - keep an `extractTo()` call rather than rewriting it into the loop, which is where the exposure actually is
- Test - assert a legitimate subdirectory read still succeeds, that `../../../etc/passwd` and a sibling such as `../documents_backup/notes.txt` are refused, and that a missing file is reported as "not found". An unchecked `realpath()` failure does not open a hole - with the candidate as the haystack, `str_starts_with(false, $base)` coerces to `str_starts_with('', $base)` and returns false, and under `declare(strict_types=1)` it raises a `TypeError` instead. Both fail closed; the damage is that every legitimate request is rejected or fatals, so test the `=== false` branch for availability rather than for escape

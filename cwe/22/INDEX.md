...
- Canonicalization that follows symlinks needs the path to exist, so it cannot validate a destination that has not been created yet and will fail outright on an upload. For a write, canonicalize the parent directory - which does exist - confirm the parent is inside the root, and require the remaining filename to be a single path component. That single-component rule is for an upload filename only: archive entries are nested by design (`sub/dir/file.txt`), and are checked by containing each entry's resolved target, not by rejecting separators
...
- Reject, do not strip - where the code tests the raw input, refuse a value containing `..`, an absolute path, or an encoded/Unicode traversal sequence rather than removing it; a single non-recursive replacement turns `....//` into `../` and a silent strip hides the attempt from logs. Canonicalize-then-contain is the fix, and once it is in place a separate `..` test is redundant and rejects a legitimate name that merely contains the substring (`notes..v2.txt`) - do not add one beside a containment check
- Test - confirm a legitimate nested path still resolves, and that a traversal sequence, an absolute path, and a path naming a sibling of the base directory are all refused; assert on the file actually opened rather than on the absence of an error

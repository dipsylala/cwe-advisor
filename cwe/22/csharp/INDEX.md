...
- `Path.Combine()` is not the safe construction step. Microsoft documents that if any argument after
  the first is rooted, every earlier component is discarded and the result begins at that rooted
  path - so an absolute path from the user replaces the base directory outright. Its reference page
  says to call `Path.Join` or `TryJoin` instead when later arguments come from user input; those

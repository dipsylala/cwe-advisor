...
- Reject with a 4xx: a passing scan is not a fix, because the platforms differ. Tomcat and Jetty replace every control character except TAB with a space as the header is written (silently, and `getContentType()` still returns the original), PHP drops a header whose value contains CR or LF mid-string - while silently trimming one that only trails, so that header is still sent - and ASP.NET Core, Node, Flask and Django raise, which is a 500 any client can trigger at will

...
- Tomcat and Jetty both neutralise on the way out, but more broadly than CR/LF: each replaces every control character except TAB with a space as the header is written (Tomcat also DEL, Jetty also any code point above 0xFF; Jetty uses `.` rather than a space inside header *names*). It is silent, nothing is logged, and `response.getContentType()` returns the value the application set, not the value the container wrote

...
- `operator[]` performs no bounds check on `std::vector`, `std::array`, `std::string`, or `std::span`; `.at()` performs it and throws `std::out_of_range`. `std::map`/`std::unordered_map::operator[]` is a different trap, not this one - a missing key gets a value-initialized element inserted rather than an out-of-bounds read, so use `.at()` or `.find()` there instead. Reserve `operator[]` for an index already validated or structurally guaranteed, such as a loop bounded by `.size()`
...
- Harden configuration - build and test with the standard library's hardened mode: `-D_GLIBCXX_ASSERTIONS` on libstdc++ (already the default at `-O0`; pass it explicitly once optimization is enabled), `-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_FAST` on libc++ (its `valid-element-access` check already covers `operator[]` writes; `extensive` adds checks unrelated to element access), and the Microsoft STL's debug container checks (`_ITERATOR_DEBUG_LEVEL` defaults to 2 in debug builds but 0 in release, so set it explicitly if the checks are wanted there), which turn an out-of-range subscript into a deterministic abort

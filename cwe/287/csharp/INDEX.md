...
- Resolve the account yourself with `UserManager.FindByNameAsync` and call the `TUser` overload of `PasswordSignInAsync`: the `string userName` overload returns `SignInResult.Failed` as soon as `FindByNameAsync` comes back null, so the unknown-name branch never reaches the hasher.
...
- Set `TokenValidationParameters.ValidAlgorithms` to the exact algorithm(s) the issuer signs with (e.g. `new[] { SecurityAlgorithms.HmacSha256 }`) so a token cannot switch to a different or weaker algorithm than intended. Unset it defaults to `null`, which does not accept everything - validation falls back to what the resolved signing key supports - but leaves the accepted set decided by key type rather than configuration.
...
- From .NET 8 the bearer handler validates with `JsonWebTokenHandler` from `Microsoft.IdentityModel.JsonWebTokens`, selected through `JwtBearerOptions.TokenHandlers`; `System.IdentityModel.Tokens.Jwt`/`JwtSecurityTokenHandler` is the legacy path. `TokenValidatedContext.SecurityToken` is a `JsonWebToken` there, so code casting it to `JwtSecurityToken` compiles and fails at run time - read claims from `context.Principal`
- Check the package version while fixing: CVE-2024-21319, a denial of service through JWE decompression, is fixed in `Microsoft.IdentityModel.JsonWebTokens`/`System.IdentityModel.Tokens.Jwt` 7.1.2, 6.34.0 and 5.7.0
...
- Break taint after allowlist validation - Populate `ClaimsPrincipal`/`HttpContext.User` only from a token that passed full `TokenValidationParameters` validation, never from a manually parsed `JwtSecurityToken`/`JsonWebToken`

...
- For resource-server JWT validation with `NimbusJwtDecoder` (`org.springframework.security.oauth2.jwt`, artifact `spring-security-oauth2-jose`), pin the algorithm on the builder actually in use: `jwsAlgorithm(...)` on `withJwkSetUri(...)`, `signatureAlgorithm(...)` on `withPublicKey(...)`, `macAlgorithm(...)` on `withSecretKey(...)` - so the accepted algorithm comes from configuration rather than the token's own header.

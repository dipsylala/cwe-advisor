...
- Validate a URL by parsing it: check `scheme` and `host` on the resulting `URI`, not on the raw string, and prefer `URI.create()` over `new URL(String)`, which is `@Deprecated(since="20")` - a soft deprecation, not marked for removal, so it will not stop compiling. The reason to move is behavioural: `URL.equals`/`hashCode` perform DNS resolution, so comparing two URLs makes a network call and can equate different hostnames that resolve alike

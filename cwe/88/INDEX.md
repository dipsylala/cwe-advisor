...
- Treat `--` as a second layer, not the control - support is thinner than it looks. curl documents it; for git it depends on position - `--` closes a value used as a path, while a value in revision position needs `--end-of-options` (2.24+), which gitcli calls an alias for `--` where a command does not separate revisions from paths; tar's manual documents no terminator at all; rsync and ssh document none. GNU `find` documents that `--` *does not work* for it, because it decides where starting points end by reading until an argument begins with `-`, and names its own remedies instead: prefix the value with `./`, pass an absolute path, or use `-files0-from`

...
- `strerror`, `localtime`, `asctime`, `getpwnam` and similar return pointers to static buffers, so a handler using them overwrites what the interrupted code was reading; the `_r` variants are reentrant but still not necessarily async-signal-safe
- Use `write(STDERR_FILENO, msg, len)` for handler output, and `_exit()` rather than `exit()` (which runs `atexit` handlers and flushes streams)
...
`printf()`/`fprintf()`/`puts()` in a handler, `malloc()`/`free()`, `exit()`, `strerror()`, `localtime()`/`asctime()`, `getpwnam()`, any mutex operation, any function not on the POSIX async-signal-safe list
...
- Bind, encode, validate, or authorize - initialise the `struct sigaction` with `sigemptyset(&sa.sa_mask)`, block handled signals with `sigaddset()`, and set `SA_RESTART`

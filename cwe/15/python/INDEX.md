...
- Load configuration once at startup with Pydantic's `BaseSettings` (`model_config = {"frozen": True}`) - in Pydantic v2 this class moved out of core `pydantic` into the separate `pydantic-settings` package - or an equivalent Flask `Config` class/Django settings module populated from environment variables, never from `request.form`/`request.json`
...
- Treat a request-reachable config write as high severity because of which keys it reaches: `SECRET_KEY` (forges sessions), `SESSION_COOKIE_SECURE`/`SESSION_COOKIE_HTTPONLY`/`SESSION_COOKIE_SAMESITE`, and Django's `ALLOWED_HOSTS`. Flask's `DEBUG` is a narrower case than it looks: Werkzeug's interactive debugger is wrapped around the app once, inside `run_simple()`, only when the dev server starts - flipping `app.config['DEBUG']` from a running request handler changes `PROPAGATE_EXCEPTIONS` behavior but does not retroactively install the console; the console risk is real only for DEBUG set before startup or a production deployment that still runs the dev server

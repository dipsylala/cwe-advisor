...
- Treat a request-reachable write to `process.env` as high severity, but check which variable is actually live for the running process: `NODE_TLS_REJECT_UNAUTHORIZED` is read fresh on every TLS handshake, so a runtime write disables certificate verification for the very next HTTPS connection. `NODE_OPTIONS` and `NODE_EXTRA_CA_CERTS` are both documented as read only once, at process start - Node's own docs state outright that "changing the value at runtime ... has no effect on the current process" for `NODE_EXTRA_CA_CERTS`, and the same holds for `NODE_OPTIONS`. A mutation to either still matters, but only for a child process spawned afterward with `child_process.spawn()`/`fork()` (which inherit `process.env` by default), not for the current process serving the request
...
- Use `zod` (or a similar schema library) with `z.enum([...])` for the allowed setting names so unknown keys are rejected before reaching `configService.set()` - validate key and value together with `z.discriminatedUnion('key', [...])`, not as two independent fields, or a valid key paired with a mismatched value type still passes

...
- Replace with the safe pattern - switch to Jakarta Mail's `Session`/`Transport`/`MimeMessage` API (or the equivalent maintained client library for IMAP/FTP) - `Session` and `Transport` live in `jakarta.mail`; `MimeMessage`, `InternetAddress` and `AddressException` in `jakarta.mail.internet`, which `import jakarta.mail.*` does not reach; the 1.6 line uses `javax.mail` and `javax.mail.internet`. For IMAP, a `Store` from `session.getStore("imaps")` must be `connect(host, user, password)`ed before `getFolder(name)` - `getFolder()` on an unconnected store throws `IllegalStateException` on every call; for FTP, Apache Commons Net's `FTPClient` (`org.apache.commons.net.ftp`) needs `connect()` then `login()` before `retrieveFile(remote, outputStream)` - and it frames nothing: `FTP.buildMessage()` (commons-net 3.11.1) appends the command, a space, the argument and CRLF with no check, so a filename containing `\r\n` still injects a second command through the library. The library replaces the raw socket; rejecting CR and LF in the filename before the call is what closes the vector. A rewrite that takes a `Session` where the caller passed an open `Socket` is a signature change to state, and the message body, subject and recipients the original sent stay as they were
...
- Harden configuration - `mail.smtp.starttls.enable=true` is opportunistic and continues in plaintext when the server does not advertise STARTTLS, so set `mail.smtp.starttls.required=true` (JavaMail 1.4.2+) to fail instead, configure a trust store, and authenticate with least-privilege credentials. This is configuration hardening beside the fix: a relay that ran in plaintext stops working against a server without STARTTLS, so state it as a separate change rather than adding it silently to the rewrite

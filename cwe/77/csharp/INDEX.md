...
- **Primary defence:** use StackExchange.Redis's typed methods (`IDatabase.StringSet`, `HashSet`, etc.) or `IDatabase.Execute(command, args)` with arguments passed as separate array elements, never concatenated into one string. `Execute` takes `params object[]`: pass the values as separate arguments or as an `object[]` - a `RedisValue[]` is not an `object[]`, so it binds as one element, and the client throws `InvalidCastException` when it serializes that element - no command is sent, and every call fails
...
- Memcached is not the same shape, so the reasoning does not carry across: only its data block is length-delimited, while the key sits on a space-delimited, CRLF-terminated command line. Safety there comes from the client rejecting the byte, not from framing - EnyimMemcachedCore's default `DefaultKeyTransformer` throws `ArgumentException` for 0x00-0x20 and space, and is replaceable via `options.KeyTransformer`. Its client types `IMemcachedClient`/`MemcachedClient` are in `Enyim.Caching`, while `StoreMode` is in `Enyim.Caching.Memcached` - a file that imports only the first does not compile

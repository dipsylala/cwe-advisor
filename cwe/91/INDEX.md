...
MITRE's name also covers XPath and XQuery injection. Where the finding is a value concatenated into an XPath *query* rather than into an XML document, read CWE-643, which carries the binding APIs per language and the reason escaping is unavailable. This is **not** the entry for XML External Entity processing: a finding about `DOCTYPE`, external entities, or entity expansion is CWE-611, and hardening a parser does nothing for a document built by concatenation.

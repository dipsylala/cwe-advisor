...
- A `.bat`/`.cmd` target re-enters `cmd.exe` on Windows even without `shell: true`; Node.js addressed the resulting injection in 2024 (CVE-2024-27980), so keep the runtime patched and prefer invoking the executable the batch file wraps

...
- On Windows a `.bat`/`.cmd` target re-enters `cmd.exe` even with `bypass_shell` set, because `CreateProcess` starts the shell for a batch file. PHP's first fix for `proc_open()` with an argument array shipped in the 8.1.28 and 8.2.18 security releases (CVE-2024-1874); sources disagree on whether the 8.3 branch got it in 8.3.5 or 8.3.6. That fix was then bypassed with a trailing space and corrected in 8.1.29, 8.2.20 and 8.3.8 (CVE-2024-5585), so set the floor at 8.1.29 / 8.2.20 / 8.3.8 and the ambiguity stops mattering - the first fix alone is bypassable anyway. The patch does not reach `exec()`, `system()`, `shell_exec()` or backticks, which invoke a shell by design and are unaffected either way, and it cannot help when the batch file itself interpolates `%1` into a further command - invoke the executable the batch file wraps

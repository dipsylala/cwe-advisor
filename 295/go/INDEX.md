...
```go
// SAFE: default validation, no custom TLS config
client := &http.Client{}
resp, err := client.Get("https://api.example.com/data")

// SAFE: internal CA added to trust pool, default validation otherwise preserved
certPool, err := x509.SystemCertPool()
if err != nil {
    certPool = x509.NewCertPool()
}
certPool.AppendCertsFromPEM(internalCAPEM)

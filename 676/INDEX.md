...
- Replace unsafe string functions with safer alternatives: strcpy → snprintf/strlcpy where available, gets → fgets, sprintf → snprintf; avoid `strncpy` unless truncation and null termination are handled explicitly
...
- Replace systematically - Use snprintf, strlcpy where available, fgets, subprocess with list arguments, and parameterized queries

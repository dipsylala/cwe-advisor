...
Path Traversal in PHP appears where a request value is concatenated into a path passed to `file_get_contents()`, `readfile()`, `fopen()`, `unlink()` or `copy()` - PHP has no path-joining function that enforces a base directory, so `$base . '/' . $_GET['file']` does exactly what it says. The canonicalizing function is `realpath()`, which resolves `.`, `..` and symbolic links but returns `false` unless every component of the path already exists. That makes it correct for validating a read and wrong for validating a write destination, which is the single most common source of broken fixes for this CWE in PHP.
...
- For writes, the destination does not exist yet, so resolve the *parent* with `realpath()` and reject a filename that is empty, `.`, `..`, or contains `/`, `\` or a null byte; open with mode `'x'`/`'xb'` so an existing file or planted symlink is refused rather than truncated
...
- Harden configuration - add `open_basedir` for the application's directories and confirm hand-rolled archive extraction loops apply the same checks that `ZipArchive::extractTo()` applies internally
- Test - assert a legitimate subdirectory read still succeeds, that `../../../etc/passwd` and a sibling such as `../documents_backup/notes.txt` are refused, and that a missing file raises "not found" rather than a `TypeError` from `false` reaching `str_starts_with()`

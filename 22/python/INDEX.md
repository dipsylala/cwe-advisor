...
- Reads and writes differ by `strict=`: `resolve(strict=True)` raises `FileNotFoundError` for a path that does not exist, which is correct for a download and wrong for an upload destination. For a write, resolve the *parent*, reject a filename containing `/`, `\`, or equal to `''`/`.`/`..`, and create with `os.O_CREAT | os.O_EXCL` so an existing file or planted symlink is refused rather than overwritten

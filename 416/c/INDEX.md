...
- Where a pointer must outlive the call that hands it out - a deferred callback, a queued job - pass a handle the receiver can revalidate rather than a raw address. A bare index is not a handle: once the slot is reused the index resolves to a different, live object and the revalidation passes. Store a generation counter in the slot, increment it both on release and on re-acquire, pass index plus generation, and accept only when both match; size the counter so it cannot wrap within the process lifetime

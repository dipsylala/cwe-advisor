...
- A lambda capturing by reference, or capturing a raw `this`, outlives the frame it referred to when it is stored in a callback or queue; capture a `shared_ptr` (or `weak_ptr`) instead

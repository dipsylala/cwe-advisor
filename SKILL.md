...
Some entries route rather than remediate: MITRE marks the ID Discouraged or Prohibited for mapping, or marks it a Class Allowed-with-Review and asks that a better-fitting child be used, so the guidance names a more specific child weakness instead of carrying the fix (CWE-20, CWE-119, CWE-200, CWE-269 and CWE-327 are the common cases). When the loaded guidance routes to a child CWE that matches the finding, read that child's `cwe/{CWE_ID}/INDEX.md` as well, treat it as the primary guidance, and use the child's ID for the language lookup in Step 3. Take one hop only, and keep the parent's guidance for context. If the child's directory doesn't exist, continue with the parent.
...
Then check it the way a compiler would - with the compiler, where one is reachable. Copy the affected files to a scratch location outside the codebase (never edit the developer's files to test a fix), apply the fix there, and run the language's checker on the result: `javac`, `node --check` (for an ES module, `node --input-type=module --check < file` - bare `--check` passes any file Node detects as ESM without parsing it), `go vet`, `php -l`, `python -m py_compile`, or the project's own build. Read every diagnostic before dismissing it. Where no checker is reachable, do the read by hand: every new symbol imported or declared exactly once, every argument list matching the definition you found, every caller in the Step 4 trace of anything whose name, parameters, or return type changed updated in the fix as well. Either way, state in the output which check ran and what it returned.

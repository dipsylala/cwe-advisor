...
- **Handle false-positive claims** - if the developer provides evidence that the finding is a false positive (e.g., the input is already validated upstream, the sink is unreachable), re-examine the data flow with that context. Step 4 governs the determination itself; what belongs here is the delivery - acknowledge a confirmed false positive plainly rather than defending the original finding, and suggest suppressing it with a documented justification.

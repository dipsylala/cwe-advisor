...
- Build tests with `-fsanitize=address,undefined` and the standard library's hardened mode (`-D_GLIBCXX_ASSERTIONS`, `-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_EXTENSIVE`), which check `operator[]` too

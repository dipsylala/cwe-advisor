...
- The attribute chain is what the disclosure runs on: `{0.__class__}` traverses attributes, while `[...]` calls `__getitem__`, so `{user[__init__]}` raises `TypeError` on a non-subscriptable object. Assume the dotted form is reachable
...
- Pass the *value* rather than the object where a template must be applied to data: `template.format(user.name)` exposes a string; `template.format(user)` exposes everything reachable from `user`
...
- Test - submit `{0.__class__.__init__.__globals__}`, `{config[SECRET_KEY]}`, and `{:>1000000000}` through every path that formats user text and confirm they are rendered literally or rejected
